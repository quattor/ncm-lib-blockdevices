...
init_raid;
$fh = EDG::WP4::CCM::Fetch->new ({PROFILE => "http://uraha.ft.uam.es/spec/hwraid3.xml",
				  FOREIGN => 1});
$fh->fetchProfile;
$cm = EDG::WP4::CCM::CacheManager->new ($fh->{CACHE_ROOT});
$cfg = $cm->getLockedConfiguration (0);
$disk = NCM::Disk->new ("/system/blockdevices/physical_devs/sdb", $cfg);
is ($disk->create, 0, "HW RAID array created when several arrays are expected");
$disk = NCM::Disk->new ("/system/blockdevices/physical_devs/spares", $cfg);
is ($disk->devpath, "/dev/spares", "Disk has the correct device name");
is ($disk->create, 0, "Wow!");
